        ...
        if cfg.degree_out(v) == 2:
            ...
            if cond:
                ...
                if len(f_v_preds) != 1 or len(t_v_preds) != 1:
                    _log.warn("ifelse: %s, %s, %s, %s is part of abnormal selection", v, t_v, f_v, f_v_s[0])
                    continue
        ...
        if type(block) is BBlock and cfg.degree_out(v) == 1:
            ...
            if isinstance(succ_block, IfElse) \
              ...
              and type(succ_block.branches[0][IFELSE_BRANCH]) is BBlock:
                ...
                _log.info("ifelse_unjumped: first: defs: %s | second: defs: %s, uses: %s", first_defs, second_defs, second_uses)
                if not first_defs:
                    # Everything was apparently DCEed
                    return
                if not first_defs.issubset(second_defs):
                    _log.info("ifelse_unjumped: can't apply, because first defines more other vars than 2nd: %s vs %s",
                         first_defs, second_defs)
                    return
                if first_defs & second_uses:
                    _log.info("ifelse_unjumped: can't apply, because if uses (%s) vals defined in preceding block (%s)",
                        second_uses, first_defs)
                    return
                # TODO: Are the checks above enough?
                _log.info("ifelse_unjumped: %s, %s", v, succ)

...
def expr_subst(expr, subst_dict):
    ...
    if isinstance(expr, REG):
        ...
        if new and expr in new:
            _log.debug("Trying to replace %s with recursively referring %s, not doing" % (expr, new))
            return None
        if new and len(new) > 10:
            _log.debug("Trying to replace %s with complex [len=%d] %s, not doing" % (expr, len(new), new))
            return None
        return new
